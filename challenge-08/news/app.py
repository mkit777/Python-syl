...
class File(db.Model):
    ...
    def exists(self):
        if collection.find_one(self.mongokey) is None:
            return False
        return True

    def add_tag(self,tag_name):
        if not self.exists():
            collection.insert_one({'fileid':self.id,'tags':[tag_name]})
        else:
            tags =collection.find_one(self.mongokey).get('tags')
            tags.append(tag_name)
            collection.update(self.mongokey,{'$set':{'tags':tags}})
    
    @property
    def mongokey(self):
        return {'fileid':self.id} 
    ...
    def remove_tag(self,tag_name):
        ...
        try:
            ...
            print('tag_name is None')
            return
        doc.update(self.mongokey,{'$set':{'tags':tags}})
    
    def speak(self):
        print('hello')
    @property
    def tags(self):
        return collection.find_one(self.mongokey).get('tags')
